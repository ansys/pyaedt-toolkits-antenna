...
class TestClass(BasisTest, object):
    ...
    def test_03_corrugated_horn(self):
        ...
        assert GeometryOperators.points_distance(face_center_eval, face_center_new) < 1e-3
        ohorn2 = self.aedtapp.add_from_toolkit(
            CorrugatedHorn, draw=True, antenna_name=ohorn1.antenna_name
        )
        assert ohorn1.antenna_name != ohorn2.antenna_name

    def test_04_elliptical_horn(self):
        self.aedtapp = self.add_app(design_name="elliptical_horn")
        self.aedtapp.solution_type = "Modal"
        ohorn1 = self.aedtapp.add_from_toolkit(
            EllipticalHorn, draw=True, frequency=10.0, huygens_box=True
        )
        assert ohorn1
        assert ohorn1.object_list
        for comp in ohorn1.object_list.values():
        ...
        assert GeometryOperators.points_distance(face_center_eval, face_center_new) < 1e-3
        ohorn2 = self.aedtapp.add_from_toolkit(
            EllipticalHorn, draw=True, antenna_name=ohorn1.antenna_name
        )
        assert ohorn1.antenna_name != ohorn2.antenna_name

    def test_05_eplane_horn(self):
        self.aedtapp = self.add_app(design_name="eplane_horn")
        self.aedtapp.solution_type = "Modal"
        ohorn1 = self.aedtapp.add_from_toolkit(
            EPlaneHorn, draw=True, frequency=10.0, huygens_box=True
        )
        assert ohorn1
        assert ohorn1.object_list
        for comp in ohorn1.object_list.values():
        ...
        assert GeometryOperators.points_distance(face_center_eval, face_center_new) < 1e-3
        ohorn2 = self.aedtapp.add_from_toolkit(
            EPlaneHorn, draw=True, antenna_name=ohorn1.antenna_name
        )
        assert ohorn1.antenna_name != ohorn2.antenna_name

    def test_06_hplane_horn(self):
        self.aedtapp = self.add_app(design_name="hplane_horn")
        self.aedtapp.solution_type = "Modal"
        ohorn1 = self.aedtapp.add_from_toolkit(
            HPlaneHorn, draw=True, frequency=10.0, huygens_box=True
        )
        assert ohorn1
        assert ohorn1.object_list
        for comp in ohorn1.object_list.values():
        ...
        assert GeometryOperators.points_distance(face_center_eval, face_center_new) < 1e-3
        ohorn2 = self.aedtapp.add_from_toolkit(
            HPlaneHorn, draw=True, antenna_name=ohorn1.antenna_name
        )
        assert ohorn1.antenna_name != ohorn2.antenna_name
