...
if [ $dependencies_available = true ]; then
    dpkg -x ./antenna_toolkit.deb ${HOME}/.local
    ./postInstallScript.sh
    available=$(cat ~/.bashrc | grep -zoP "# Add alias for Ansys Antenna Toolkit \nalias antenna_toolkit=~/.local/opt/antenna_toolkit/antenna_toolkit" | wc -l)
    echo $available
    if [ $available -lt 1 ]
    then
        echo -e "# Add alias for Ansys Antenna Toolkit \nalias antenna_toolkit=~/.local/opt/antenna_toolkit/antenna_toolkit" >> ~/.bashrc
    fi
